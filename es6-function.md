...
但是严格模式又跟参数默认值、解构赋值、运算符等有冲突
正常模式也可以使用 只能用循环来模拟递归 达到我们的目的 只要一次`调用记录`
可以[参考](https://es6.ruanyifeng.com/#docs/function#%E5%B0%BE%E8%B0%83%E7%94%A8%E4%BC%98%E5%8C%96)阮一峰大神的ECMAScript 6 入门

## 总结
* ES6的函数的参数可以设置默认值 也可以用解构赋值  参数默认值是计算的结果 所以有自己独特的作用域
* ES6新增了`rest`参数`...` 用来获得多余的参数 最终得到一个数组的集合
* 函数内部可以通过`'use strict'` 设置成严格模式 但是参数的`默认值`，`对解构赋值`，`运算符`就不能用了
* ES6的函数属性 `length` 如果有参数默认值 和 rest参数 就会失真
* ES6新增的箭头函数，函数内的`this`是固定的 哪个定义就是哪个 不再是使用的对象。不能用来定义构造函数(`new`)
